...
export const get = async (path, queryKey = '', queryValue = '') => {
...
export const getWatch = (path, callback, queryKey = '', queryValue = '') => {
  let unsubFn = null;
  if(queryKey === '' || queryValue === ''){
...
export const push = async (path, data) => {
  try{
    await push_db(ref(database, path), data);
    return true;
  }catch(error){
    console.error(error);
    return false;
